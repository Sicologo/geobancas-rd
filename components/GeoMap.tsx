...
function protectedGeoJSON(items:ProtectedPlace[]):GeoJSON.FeatureCollection<GeoJSON.Point>{return{type:"FeatureCollection",features:items.map(x=>({type:"Feature",id:x.id,geometry:{type:"Point",coordinates:[x.lng,x.lat]},properties:{id:x.id,nombre:x.nombre,categoria:x.categoria}}))}}
 ...
 useEffect(()=>{if(!el.current||mapRef.current)return;
  ...
  map.on("load",()=>{
   ...
   const addLayerSet=(id:string,data:Banca[]|Escuela[]|Salud[]|Destacamento[],kind:"banca"|"escuela"|"salud"|"destacamento",color:string,visible:boolean)=>{
    ...
    // Todos los tipos deben ser visibles desde la vista nacional. Antes las capas
    // protegidas arrancaban en zoom 11.2 y quedaban completamente ocultas.
    const clusterMinZoom=6.6;
    map.addLayer({id:`${id}-clusters`,type:"circle",source:id,minzoom:clusterMinZoom,filter:["has","point_count"],layout:{visibility},paint:{"circle-color":isBanca?"rgba(8,17,29,.88)":color,"circle-radius":isBanca?["step",["get","point_count"],10,25,13,100,16,500,20]:["step",["get","point_count"],9,20,11,75,14],"circle-stroke-width":1,"circle-stroke-color":isBanca?"rgba(34,201,244,.55)":"rgba(255,255,255,.92)","circle-opacity":.92}});
    map.addLayer({id:`${id}-count`,type:"symbol",source:id,minzoom:clusterMinZoom,filter:["has","point_count"],layout:{visibility,"text-field":["get","point_count_abbreviated"],"text-size":isBanca?9:8,"text-font":["Open Sans Bold"],"text-allow-overlap":true},paint:{"text-color":"#f8fbff","text-halo-color":"rgba(8,17,29,.4)","text-halo-width":.6}});
    if(isBanca){
     // Fallback visual permanente: garantiza que las bancas sigan visibles incluso si un icono personalizado falla al cargarse.
     map.addLayer({id:`${id}-blip`,type:"circle",source:id,minzoom:9.4,filter:["!",["has","point_count"]],layout:{visibility},paint:{"circle-color":["match",["get","estatus"],"Legal","#22c55e","Ilegal","#ef4444","Pendiente","#facc15","Suspendida","#94a3b8","#facc15"],"circle-radius":["interpolate",["linear"],["zoom"],9.4,3.2,12,4.4,15,5.6,18,7.2],"circle-stroke-width":["interpolate",["linear"],["zoom"],9.4,.8,15,1.3],"circle-stroke-color":"rgba(255,255,255,.92)","circle-opacity":.96}});
     map.addLayer({id:`${id}-point`,type:"symbol",source:id,minzoom:12.2,filter:["!",["has","point_count"]],layout:{visibility,"icon-image":["match",["get","estatus"],"Legal","geo-banca-legal","Ilegal","geo-banca-ilegal","Pendiente","geo-banca-pendiente","Suspendida","geo-banca-suspendida","geo-banca-pendiente"],"icon-size":["interpolate",["linear"],["zoom"],12.2,.48,13.1,.62,15,.82,18,1.06],"icon-allow-overlap":true,"icon-ignore-placement":true,"symbol-sort-key":2},paint:{"icon-opacity":.98}});
    }else{
     const icon=kind==="escuela"?"geo-school":kind==="salud"?"geo-health":"geo-police";
     // Fallback circular para escuelas, salud y policía. Así nunca desaparecen
     // aunque el icono personalizado no llegue a renderizarse por colocación.
     map.addLayer({id:`${id}-blip`,type:"circle",source:id,minzoom:10.8,filter:["!",["has","point_count"]],layout:{visibility},paint:{"circle-color":color,"circle-radius":["interpolate",["linear"],["zoom"],10.8,3.2,13,4.6,16,5.6,18,6.5],"circle-stroke-width":1.2,"circle-stroke-color":"rgba(255,255,255,.95)","circle-opacity":.96}});
     map.addLayer({id:`${id}-point`,type:"symbol",source:id,minzoom:11.6,filter:["!",["has","point_count"]],layout:{visibility,"icon-image":icon,"icon-size":["interpolate",["linear"],["zoom"],11.6,.58,14,.74,18,.94],"icon-allow-overlap":true,"icon-ignore-placement":true,"symbol-sort-key":1},paint:{"icon-opacity":.98}});
    }
   };

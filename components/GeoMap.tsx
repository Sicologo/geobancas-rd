 ...
 useEffect(()=>{if(!el.current||mapRef.current)return;
  ...
  map.on("load",()=>{
   ...
   const addLayerSet=(id:string,data:Banca[]|Escuela[]|Salud[]|Destacamento[],kind:"banca"|"escuela"|"salud"|"destacamento",color:string,visible:boolean)=>{
    ...
    const clusterMinZoom=isBanca?6.6:11.2;
   ...
   ["bancas-clusters","escuelas-clusters","salud-clusters","destacamentos-clusters","bancas-blip","bancas-point","search-results","escuelas-point","salud-point","destacamentos-point"].forEach(id=>{map.on("mouseenter",id,()=>map.getCanvas().style.cursor="pointer");map.on("mouseleave",id,()=>map.getCanvas().style.cursor="")});
